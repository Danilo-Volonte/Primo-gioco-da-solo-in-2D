# SPIEGAZIONE DEL GIOCO
Il gioco ha due modalità: Battle Royale e Racing. <br>
La prima è una Battle royal come tutte le atre. <br>
Mentre il secondo è una modalità di gioco dove si gareggia in una gara in un ciruito, dove ci si potrà sparare o colpirsi a vicenda. <br>
Il gioco avrà vari tipi di macchine:
- RALLY
    - Velocità: media-alta
    - Accelerazione: alta
    - Maneggevolezza: alta
    - Robustezza: media-bassa
    - Resistenza ai Danni: alta in caso di incidenti tra veicoli o altre cose e media bassa per tutti gli altri Danni (proiettili, bombe)
    - Resistenza a fatica: media-Hbassa
- BUGGY
    - Velocità: alta
    - Accelerazione: alta
    - Maneggevolezza: alta
    - Robustezza: carrozeria molto resistente ma le gomme no essendo che son fuori
    - Resistenza ai Danni: carrozeria ottimo ma poco resistente nelle gomme
    - Resistenza a fatica: media
- CAMION
    - Velocità: bassa
    - Accelerazione: media
    - Maneggevolezza: bassa
    - Robustezza: media
    - Resistenza ai Danni: media bassa per incidenti me alta per proiettili
    - Resistenza a fatica: media-alta
- FUORISTRADA
    - Velocità: bassa
    - Accelerazione: bassa
    - Manegevolezza: media-bassa
    - Robutezza: media su tuto tranne che su motore cheinvece è alto
    - Resistenza ai Danni: medio tranne su mototore che è alto
    - Resistenza a fatica: alta

Nel gioco ogni macchina avrà delle specifiche armi, che sono di tipo:
- CECCHINO
    - Danni per proiettile: 100
    - Precisione: altssima
    - Rinculo: alto
    - Stabilità: bassa
    - Manegevolezza: bassa
    - Velocità di ricarica: lenta
    - Cadenza di fuoco: lenta
    - Gittata: qualsiasi distanza
- POMPA
    - Danni per proiettile: 100 a distanza ravvicinata e 50 a media
    - Precisione: media-bassa
    - Rinculo: medio-alto
    - Stabilità: media-bassa
    - Manegevolezza: media
    - Velocità di ricarica: media
    - Cadenza di fuoco: media-lenta
    - Gittata: a distanza ravvicinate e massimo medie
- ARMI D'ASSALTO 
    - Danni per proiettile: 30
    - Precisione: media-alta
    - Rinculo: medio-basso
    - Stabilità: media-alto
    - Manegevolezza: media
    - Velocità di ricarica: veloce
    - Cadenza di fuoco: molto veloce
    - Gittata: qualsiasi distanza ma sopratutto a medie distanze
- MITRAGLIATRICI
    - Danni per proiettile: 30
    - Precisione: media
    - Rinculo: medio-basso
    - Stabilità: media-bassa
    - Manegevolezza: bassa
    - Velocità di ricarica: lenta
    - Cadenza di fuoco: velocissima
    - Gittata: qualsiasi distanza

# SPIEGAZIONE GAMEPLAY
Per tutte e due le modalità i pulsanti saranno gli stessi: wasd o le freccette per guidare con la tastiera. <br> 
Però il gioco è più bello da giocare con il controller, per qua i tasti dovranno essere testati successivamente. <br>
Come detto prima la modalità Battle Royale è come quella di altri giochi, quindi ci sarà una mappa abbastanza grande dove tanti giocatri si sfideranno tra loro per poter sopravvivere e vincere. <br>
Vince chi sopravvive a tutti i giocatori. <br>
Oltre a ciò c'è la classica tempesta che si rimpicciolisce, rimpicciolendo la safezone dove si può combattere senza problemi. <br>
Chi entra nella tempesta avrà una spinta circolare o destra o a sinistra. <br>
Le macchine più pesanti saranno soggete meno a questa cosa mentre le più leggere saranno colpute molto più duramente. <br>
In più più si sta dentro più danni si prendono per fare in modo che la tempesta non venga utilizzata per ottenere vantaggio senza richiare niente. <br>

La modalità Racing invece è semplicemente una gara simile al rallycross dove vince chi acquista più punti. <br>
I punti si possono aquistare o vincendo la gara o facendo esploedere i propri avversari. <br>
Ovviamente vincere fa guadaganre molti più punti che sconfiggendo avversari ma questo non impedisce a un giocatore di poter vicere senza essere arrivato prima a tante gare. <br>
